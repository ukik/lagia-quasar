...
export const useAudioListStore = defineStore('audio-list-favorite', {
  state: () => ({
  ...
  actions: {
    ...
    async request (payload = null) {
      ...
      const results = await axios.get(`api/audio/favorite/${user_id}?page=1`)
      .catch(err => {
        console.log(err)
        errorNotify()
        return null
    ...
    async request_more () {
      ...
      const results = await axios.get(`api/audio/favorite/${user_id}?page=${next_page}`)
      .catch(err => {
        errorNotify()
        return null
      })
