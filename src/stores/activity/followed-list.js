...
export const useListStore = defineStore('followed-list', {
  state: () => ({
    content: [],
    pagination,
  ...
  actions: {
    async refresh (payload = null) {
      ...
      const results = await axios.get(`api/profile/activity/followed/${user_id}?page=1`)
      .catch(err => {
        console.log(err)
        errorNotify()
        return null
    ...
    async request (payload = null) {
      ...
      const results = await axios.get(`api/profile/activity/followed/${user_id}?page=${Number(this.pagination.current_page) + 1}`)
      .catch(err => {
        console.log(err)
        errorNotify()
        return null
    ...
    async request_more () {
      ...
      const results = await axios.get(`api/profile/activity/followed/${user_id}?page=${next_page}`)
      .catch(err => {
        errorNotify()
        return null
      })
    ...
    async follow (payload = null) {
      ...
      const results = await axios({
        method: 'post',
        url: `api/profile/activity/followed/${payload.user_id}`,
      })
      .catch(err => {
        errorNotify()
        return null
