...
export const useListStore = defineStore('musisi-youtube-list', {
  state: () => ({
    content: [],
    pagination,
    loading: false,
  ...
  actions: {
    ...
    async request (payload = null) {
      ...
      const results = await axios.get(`api/profile/musisi/youtube/${slug}?page=1`)
      // const results = await axios.get(`api/profile/musisi/youtube/${slug}?page=${Number(this.pagination.current_page) + 1}`)
      .catch(err => {
        console.log(err)
        errorNotify()
        return null
    ...
    async request_more () {
      ...
      const results = await axios.get(`api/profile/musisi/youtube/${slug}?page=${next_page}`)
      .catch(err => {
        errorNotify()
        return null
      })
