...
export const useListStore = defineStore('musisi-image-list', {
  state: () => ({
    content: [],
    pagination,
    loading: false,
  ...
  actions: {
    ...
    async request (payload = null) {
      ...
      const results = await axios.get(`api/profile/musisi/image/${slug}?page=1`)
      // const results = await axios.get(`api/profile/musisi/image/${slug}?page=${Number(this.pagination.current_page) + 1}`)
      .catch(err => {
        console.log(err)
        errorNotify()
        return null
    ...
    async request_more () {
      ...
      const results = await axios.get(`api/profile/musisi/image/${slug}?page=${next_page}`)
      .catch(err => {
        errorNotify()
        return null
      })
