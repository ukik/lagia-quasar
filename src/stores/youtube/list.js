...
export const useYoutubeListStore = defineStore('youtube-list', {
  ...
  actions: {
    async home_request (payload = null) {

      if(this.loading) return

      const { category } = useRouterStore().getQuery // gak support preFetch

      this.loading = true

      const results = await axios.get(`api/youtube?page=${payload?.page}&category=${category}`)
      .catch(err => {
        errorNotify()
        return null
      })
    ...
    async home_request_more () {
      ...
      const results = await axios.get(`api/youtube?page=${next_page}&category=${category}`)
      .catch(err => {
        errorNotify()
        return null
      })
