...
export const useTourPriceListStore = defineStore('TourPriceListStore', {
  id: 'TourPriceListStore',
  ...
  actions: {
    ...
    async onFetch({ currentPage, query }) {
      ...
      const response = await axios({
        url: '/trevolia-api/v1/entities/tour-prices/lagia',
        method: 'get',
        params: {
          slug: this.slug,
          page: this.page,
      ...
      console.log('stores/lagia-stores/TourPriceListStore 1', response?.data)
      ...
      //   if(element?.tourProduct?.category) element['tourProduct']['category'] = JSON.parse(element['tourProduct']['category'])
      // });
      ...
      console.log('stores/lagia-stores/TourPriceListStore 2', this.data, this.records, this.lastPage, this.currentPage, this.perPage, this.totalItem)

      if (response?.data?.additional?.image) response.data.additional['image'] = JSON.parse(response.data.additional['image'])
      if (response?.data?.additional?.tourStore?.image) response.data.additional['tourStore']['image'] = JSON.parse(response.data.additional['tourStore']['image'])
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useTourPriceListStore, import.meta.hot));
}
