...
export const useImageListStore = defineStore('image-list-liked', {
  state: () => ({
  ...
  actions: {
    ...
    async request (payload = null) {
      ...
      const results = await axios.get(`api/image/liked/${user_id}?page=1`)
      .catch(err => {
        console.log(err)
        errorNotify()
        return null
    ...
    async request_more () {
      ...
      const results = await axios.get(`api/image/liked/${user_id}?page=${next_page}`)
      .catch(err => {
        errorNotify()
        return null
      })
