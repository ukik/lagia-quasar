<template>
  ...
  <fragment>
        ...
        <q-item-section >
          <q-item-label v-ripple="{color: 'grey' }" class="relative-position text-grey-8 q-pa-sm">
            {{ prop_data.konten }}
          </q-item-label>
          ...
          <q-toolbar class="text-grey-6 q-px-none q-mx-none" style="font-size:12px;">
            ...
            <q-btn dense class="q-px-sm" flat no-caps style="font-size:12px;" color="primary" label="Balas" />
          </q-toolbar>
    ...
    computed: {
      ...
      get_total() {
        return function(label) {
          const temp = this.prop_data.user_totals
