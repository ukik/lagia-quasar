  ...
  <q-list class="row bg-white col-12">
      ...
      <q-btn v-if="item.user?.id == auth_id" :disable="loading_delete" :loading="loading_delete" dense style="height:30px;" flat round icon="more_vert" color="dark">
        <q-menu>
          <q-list style="min-width: 100px">
            <q-item :to="goto({ name:'agenda_create', params: { id: item?.id } })" clickable v-close-popup>
          ...
          <div class="col-auto">
            ...
            <q-btn flat round icon="fa-regular fa-share-from-square" color="dark">
              <q-menu class="q-pa-md">
                <Share
                  :url="`${server_host}agenda-komentar-semua/${item.id}/1`"
                  :title="item.description"
                  :source="`${server_host}agenda-komentar-semua/${item.id}/1`"
    ...
    <template v-if="item?.comments" v-for="(value, index) in item.comments">
      ...
      <div class="col-12 row flex justify-start">
        <TombolBalasKomentar @onBubbleEventReply="() => reply = value" :id="value?.id" name="agenda_komentar_balasan" />
...
// import { useAgendaListStore } from 'src/stores/profiles/musisi-agenda-detail.js'
...
export default {
  ...
  methods: {
    ...
    onZoom(index) {
      this.$global.$emit('profile-detail', {
        type: 'tab-image',
        zoom_visible: true,
        zoom_index: index,
        zoom_images: this.item.images,
      })
    },
    onCollapse(index) {
