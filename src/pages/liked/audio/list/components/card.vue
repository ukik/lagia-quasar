<template>
  <q-list class="row bg-white col-12">
    ...
    <q-card class="bg-dark col-12" :style="is_mobile_size ? 'height:300px' : 'height:400px'">
      <q-img @error="onError" :src="imageSync(item?.cover)" class="col-12 bg-dark" :style="is_mobile_size ? 'height:300px' : 'height:400px'">
    ...
    <template v-if="item?.comments" v-for="(value, index) in item.comments">
      ...
      <div class="col-12 row flex justify-start">
        <TombolBalasKomentar @onBubbleEventReply="() => reply = value" :id="value?.id" name="audio_komentar_balasan" />
...
import { useAudioListStore } from 'src/stores/audio/list-favorite.js'
