  ...
  <q-list class="row bg-white col-12">
    <q-item class="col-12 q-px-sm">
      <q-item-section avatar>
        <q-avatar>
          <img :src="imageSync(item.user?.avatar)" @error="handleError"  style="border-radius:50px; object-fit: cover;">
        </q-avatar>
      ...
      <q-item-section>
        ...
        <q-item-label caption lines="1">
          <q-chip v-if="item?.user?.id != auth_id" :disable="loading_follow" clickable @click="onFollow" class="q-ma-none" :color="!item.user.follow ? 'positive' : 'primary'"
    ...
    <q-carousel v-if="item?.images" class="col-12" :style="[is_mobile_size ? 'height:250px;' : 'height:450px;']" swipeable animated v-model="slide"  arrows navigation infinite>
        <q-carousel-slide v-if="item?.images.length > 0" class="no-scroll q-pa-none" v-for="(el, index) in item?.images" :key="index" :name="index">
            <q-img @click="$emit('onLightBox', {
              index: index,
              images: getLightbox(item?.images,'file')
            })" :src="imageSync(el.file)" alt="mountains" class="fit" style="object-fit: cover;">
        ...
        <q-carousel-slide v-else @click="$emit('onLightBox', {
              index: index,
              images: default_image
            })" :name="0" :img-src="default_image" />
    </q-carousel>
          ...
          <div class="col-auto">
            <q-btn flat round icon="fa-regular fa-share-from-square" color="dark">
              <q-menu class="q-pa-md">
                <Share
                  :url="`${server_host}image-komentar-semua/${item.id}/1`"
                  :title="item.description"
                  :source="`${server_host}image-komentar-semua/${item.id}/1`"
    ...
    <template v-for="(value, index) in item.comments">
      ...
      <div class="col-12 row flex justify-start">
        <TombolBalasKomentar @onBubbleEventReply="() => reply = value" :id="value?.id" name="image_komentar_balasan" />
      ...
      <q-item-section>
        <q-item-label v-if="reply">
          ...
          <q-chip :to="goto({ name:'profile_musisi', params: { slug: reply?.user?.id } })" size="12px" :disable="loading_comment" class="q-mx-none" @remove="reply = null" removable color="red" text-color="white">
            <q-avatar>
              <img :src="imageSync(reply?.user?.avatar)" @error="handleError" >
            </q-avatar>
            <div class="ellipsis">
              <!-- @ -->{{ reply?.user?.name }}
            </div>
