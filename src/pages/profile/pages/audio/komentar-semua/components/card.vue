<template>
  <q-list class="row bg-white col-12">
      ...
      <q-btn v-if="item.user?.id == auth_id" :disable="loading_delete" :loading="loading_delete" dense style="height:30px;" flat round icon="more_vert" color="dark">
        <q-menu>
          <q-list style="min-width: 100px">
            <q-item :to="goto({ name:'audio_create', params: { id: item?.id } })" clickable v-close-popup>
    ...
    <!-- <q-img
      :style="is_mobile_size ? 'height:350px' : 'height:450px'" :src="item.cover" class="col-12">
      <div class="absolute-bottom full-width">
        <audio :id="`video${index}`" ref="video" :src="item.file" class="full-width"
          controls autoplay>
          Your browser does not support HTML video.
        </audio>
      </div>
    </q-img> -->
    <q-card class="bg-dark col-12" :style="is_mobile_size ? 'height:350px' : 'height:450px'">
      <q-img @click="onZoom(index)" :src="imageSync(item?.cover)" class="col-12 bg-dark" :style="is_mobile_size ? 'height:350px' : 'height:450px'">
        <template v-slot:error>
          <div class="absolute-full text-white flex flex-center bg-transparent">
            No Cover
          </div>
        </template>
      </q-img>
          ...
          <div class="col-auto">
            ...
            <q-btn flat round icon="fa-regular fa-share-from-square" color="dark">
              <q-menu class="q-pa-md">
                <Share
                  :url="`${server_host}audio-komentar-semua/${item.id}/1`"
                  :title="item.description"
                  :source="`${server_host}audio-komentar-semua/${item.id}/1`"
    ...
    <q-item class="col-12" dense>
      <q-item-section>
        <q-item-label lines="2">
          <span class="text-italic">Judul: </span>{{ item.title }},
          <span class="text-italic">Album: </span>{{ item.album }},
          <span class="text-italic">Vokalis: </span>{{ item.vocalist }}
        </q-item-label>
      </q-item-section>
    </q-item>
...
import { useDetailListStore } from 'src/stores/profiles/musisi-audio-detail.js'
  ...
  methods: {
    ...
    onZoom(index) {
      this.$global.$emit('profile-detail', {
        ...
        zoom_visible: true,
        zoom_index: 0,
        zoom_images: [{ file: this.item?.cover }],
      })
    },
    // async onComment() {
