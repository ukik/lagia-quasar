<template>
  <q-list class="row bg-white col-12">
    ...
    <!-- <q-carousel v-if="item.images.length > 0" class="col-12" :style="[is_mobile_size ? 'height:350px;' : 'height:450px;']" swipeable animated v-model="slide"  arrows navigation infinite>
        <q-carousel-slide class="no-scroll q-pa-none" v-for="(el, index) in item.images" :key="index" :name="index">
            <q-img @click="onZoom(index)" :src="imageSync(el.file)" alt="mountains" class="fit" style="object-fit: cover;">
    ...
    <q-carousel v-if="item?.images" class="col-12" :style="[is_mobile_size ? 'height:250px;' : 'height:450px;']" swipeable animated v-model="slide"  arrows navigation infinite>
      ...
      <q-carousel-slide v-else @click="onZoom(index)" :name="0" :img-src="default_image" />
    </q-carousel>
...
import { useDetailListStore } from 'src/stores/profiles/musisi-image-detail.js'
...
export default {
  ...
  methods: {
    ...
    onZoom(index) {
      console.log('onZoom',this.item?.images)
