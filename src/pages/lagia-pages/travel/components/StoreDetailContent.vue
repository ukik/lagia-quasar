<template>
  ...
  <div class="">
    ...
    <div>Dibuat pada: {{ record?.createdAt }}</div>
    ...
    <div class="col-12 q-my-md" v-if="record?.category">
      <template
        v-for="(category, category_index) in record?.category?.split(',')"
      >
        <q-chip
          color="blue-5"
          text-color="white"
    ...
    <q-list  class="rounded-borders-1 bg-grey-2 row col flex items-start text-capitalize">
        <q-item class="col-12" >
          ...
          <q-item-section side>
            <q-item-label lines="1"><a href="#">{{ record?.location }}</a></q-item-label>
          </q-item-section>
        </q-item>
        <q-item class="col-12" >
          <q-item-section>
    ...
    <q-list  class="q-mt-lg rounded-borders-1 bg-grey-2 row col flex items-start text-capitalize">
    ...
    <h3 class="q-mt-lg q-mb-sm">POLICY</h3>
    <div v-html="record?.policy"></div>

    <h3 class="q-mt-lg q-mb-sm">DESCRIPTION</h3>
    ...
    <StoreDetailPriceList :items="record?.travelPrices"></StoreDetailPriceList>
