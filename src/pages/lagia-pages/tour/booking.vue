  ...
  >
    <q-card :style="$q.screen.width > 768 ? 'width: 750px !important' : ''">
      ...
      <q-card-section style="height: calc(99.5% - 50px)" class="scroll">
        <PriceListCard
          :item="dialog_price_selected?.tourBookingItem"
          :images="dialog_price_selected?.tourStore?.tourProduct?.image"
        ></PriceListCard>
      </q-card-section>
    </q-card>
  </q-dialog>
                  ...
                  <q-item class="q-pa-none">
                    <q-item-section>
                      ...
                      <q-item-label class="text-h6">{{
                        $currency(
                          finalPrice(props.row.tourBookingItem) +
                            finalPriceAnak(props.row.tourBookingItem)
                        )
                      }}</q-item-label>
                    </q-item-section>
                  </q-item>
          ...
          <template v-slot:body="props">
            ...
            {{ props.row.id }}
              ...
              <q-td key="image" :props="props">
                <template v-if="props.row.tourStore?.tourProduct">
                  <q-img
                    @click="showMultiple(props.row.tourStore?.tourProduct.image, 0)"
                    height="100px"
                    width="100px"
                ...
                <div>
                  <q-btn-group rounded spread unelevated outline class="q-px-sm q-mt-sm">
                    ...
                    <q-btn
                      @click="
                        dialog_product = true;
                        dialog_product_selected = props.row?.tourStore?.tourProduct;
                      "
                      dense
                      size="sm"
                      label="produk"
...
import { useTourBookingListStore } from "stores/lagia-stores/tour/TourBookingListStore";
// import { useTourCartSelectedStore } from "stores/lagia-stores/tour/TourCartSelectedStore";
import { useRouter, onBeforeRouteLeave } from "vue-router";
const store = useTourBookingListStore();
...
//       return useTourBookingListStore(store).onFetch({
...
const watchPagination = async (params) => {
  console.log("watchPagination", params, search);
...
const watchSearch = async (params) => {
  console.log("watchPagination", params, search);
...
function finalPrice(item) {
  ...
  const total =
    Number(general) - (Number(general) * Number(discount)) / 100 - Number(cashback);
...
function getSubTotalExcludeHotel(props) {
  const n =
    Number(
      finalPriceAnak(props.row.tourBookingItem) *
        props.row.tourBookingItem.participantYoung
    ) +
    Number(
      finalPrice(props.row.tourBookingItem) * props.row.tourBookingItem.participantAdult
    );
