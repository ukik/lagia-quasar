  ...
  <q-no-ssr>
    ...
    >
      <q-card>
        ...
        <q-card-section style="height: calc(99.5% - 50px - 60px)" class="scroll">
          <ProductDialog :item="record"></ProductDialog>
        </q-card-section>
    ...
    >
      <q-card>
        ...
        <q-card-section style="height: calc(99.5% - 50px - 60px)" class="scroll">
          <PriceDialog :item="record"></PriceDialog>
        </q-card-section>
    ...
    >
      <q-card :style="$q.screen.width > 768 ? 'width: 750px !important' : ''">
        ...
        <q-card-section style="height: calc(99.5% - 50px)" class="scroll">
          ...
          <StoreDialogCard :item="record"></StoreDialogCard>
        </q-card-section>
      </q-card>
    </q-dialog>
  </q-no-ssr>
...
import StoreDialogCard from "./components/StoreDialogCard";
...
import { useTourStoreListStore } from "stores/lagia-stores/tour/TourStoreListStore";
import { useRouter, onBeforeRouteLeave } from "vue-router";
const store = useTourStoreListStore();
...
defineOptions({
  preFetch: preFetch(
    ({
      ...
      return useTourStoreListStore(store).onFetch({
...
const tour_prices = ref(false);
const tour_products = ref(false);

function closeDialog() {
  dialog_selengkapnya.value = false;
  record.value = null;
  tour_prices.value = false;
  tour_products.value = false;
}
