  ...
  <div class="form-box">
    ...
    >
      <q-card flat class="rounded-borders-2 bg-form">
        ...
        <q-card-actions class="col-12 text-center q-pb-lg" align="center">
            <q-btn
              type="submit"
              icon-right="send"
              outline
...
import { useCustomerStore } from "stores/lagia-stores/page/CustomerStore";
  ...
  setup() {
    ...
    return {
      ...
      async onSubmit() {
        ...
        // @click="onAddToBook"
        if(nameRef.value.hasError) return this.notify("Nama (wajib)")
        if(emailRef.value.hasError) return this.notify("Email (wajib)")
        if(phoneRef.value.hasError) return this.notify("Telepon (wajib)")
        // if(instanceRef.value.hasError) this.notify("Instansi (wajib)")
        if(cityRef.value.hasError) return this.notify("Kota Asal (wajib)")
        if(addressRef.value.hasError) return this.notify("Alamat (wajib)")
