<template>
  <q-virtual-scroll ref="virtualListRef" @virtual-scroll="onscroll" style="height: calc(100vh - 50px - 36px)"
    class="bg-grey-1" :items="get_data" separator v-slot="{ item, index }">
    <q-list>
      <q-pull-to-refresh disable v-if="index === 0" :key="index + 'C'" ref="pullToRefresh"
        @refresh="refresh">
...
export default {
  ...
  methods: {
    ...
    ...mapActions(useImageListStore, {
      ...
      request_more: 'home_request_more',
    }),
    onscroll(event) {
