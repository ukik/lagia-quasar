<template>
  <q-list class="row bg-white col-12">
    <q-item class="col-12 q-px-sm">
      ...
      <q-btn v-if="item.user?.id == auth_id" :disable="loading_delete" :loading="loading_delete" dense style="height:30px;" flat round icon="more_vert" color="dark" :class="[ is_mobile_size ? '' : 'q-mr-md']">
        <q-menu>
    ...
    <q-item class="col-12 q-pa-sm" dense>
      ...
      <q-item-section side>
        <q-btn :disable="loading_bookmarked" :loading="loading_bookmarked" @click="bookmarked({ post_id: item.id, index }); authCheck(); gotoLogin();" flat round :class="[ is_mobile_size ? '' : 'q-mr-md']"
