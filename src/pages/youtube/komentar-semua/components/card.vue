<template>
  <q-list class="row bg-white" id="mybox">
    <!-- {{ _clientWidth }} -->
      ...
      <q-btn v-if="item.user?.id == auth_id" :disable="loading_delete" :loading="loading_delete" dense style="height:30px;" flat round icon="more_vert" color="dark">
        <q-menu>
          <q-list style="min-width: 100px">
            <q-item :to="goto({ name:'youtube_create', params: { id: item?.id } })" clickable v-close-popup>
    ...
    <YouTube v-if="_clientWidth > 0" :height="is_mobile_size ? 250 : 350" :width="_clientWidth"
      :src="item.file"
      @ready="onReady"
      id="video" ref="video" />
          ...
          <div class="col-auto">
            ...
            <q-btn flat round icon="fa-regular fa-share-from-square" color="dark">
              <q-menu class="q-pa-md">
                <Share
                  :url="`${server_host}youtube-komentar-semua/${item.id}/1`"
                  :title="item.description"
                  :source="`${server_host}youtube-komentar-semua/${item.id}/1`"
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      if(document.getElementById('mybox')) this._clientWidth = document.getElementById('mybox').clientWidth
    })
  },
  methods: {
    ...
    onReady() {
      ...
      setTimeout(() => {
        const media = this.$refs.video.playVideo();
        console.log(media)
        // if(media) {
        //   media.onPlay()
        // }
